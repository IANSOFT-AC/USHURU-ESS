...
class Navhelper extends Component
{
    ...
    public function getData($service, $params = [])
    {
        ...
        if (is_string($results) || empty($results)) {
            $error = ['service' => $soapWsdl, 'error' => $results];
            Yii::$app->recruitment->printrr($error);
        }
    ...
    public function checkAuthority($payload)
    {
        // use Enployee No to Check Ownership
        if (property_exists($payload, 'Employee_No')) {
            if ($payload->Employee_No !== Yii::$app->user->identity->Employee[0]->No) {
                throw new \yii\web\HttpException(403, 'You are not authorized to view this document.');
            }
        }
    }
